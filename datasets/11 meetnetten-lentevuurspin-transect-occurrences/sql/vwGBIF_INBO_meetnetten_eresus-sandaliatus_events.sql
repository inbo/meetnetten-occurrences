...
/****** Object:  View [iptdev].[vwGBIF_INBO_meetnetten_generiek_events]    Script Date: 12/09/2019 10:33:10 ******/
SET ANSI_NULLS ON
GO
...
/* generieke query events, test met vuursalamander
   ...
   EventDate --> lentevuurspin datum is nog niet gevonden
 */

ALTER VIEW [iptdev].[vwGBIF_INBO_meetnetten_eresus-sandaliatus_events]
AS
...
SELECT --fa.*   --unieke kolomnamen
	...
	, [datasetName] = N'Meetnetten - eresus sandaliatus, Belgium'
...
--	, CONVERT(decimal(10,5), dL.LocationGeom.MakeValid().STCentroid().STX) as decimalLongitude
	, CONVERT(decimal(10,5), dL.ParentLocationGeom.MakeValid().STCentroid().STY) as decimalLatitude
	, CONVERT(decimal(10,5), dL.ParentLocationGeom.MakeValid().STCentroid().STX) as decimalLongitude
