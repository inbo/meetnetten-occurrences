...
/****** Object:  View [iptdev].[vwGBIF_INBO_meetnetten_generiek_events]    Script Date: 13/09/2019 10:44:11 ******/
SET ANSI_NULLS ON
GO
...
/* generieke query events, test met vuursalamander
   ...
   speciesActivityKey filtered for unique events??? Check
   Location is startpoint of transect

 */

ALTER VIEW [iptdev].[vwGBIF_INBO_meetnetten_dormouse-transects_events]
AS
...
SELECT --fa.*   --unieke kolomnamen
	...
	, [datasetName] = N'Meetnetten - dormouse transects, Belgium'
...
--	, [waterbody] = dL.Location
	...
	, CONVERT(decimal(10,5), dL.LocationGeom.MakeValid().STStartPoint().STY) as decimalLatitudeStart	, CONVERT(decimal(10,5), dL.LocationGeom.MakeValid().STStartPoint().STX) as decimalLongitudeStop
...
AND fa.ProtocolID =  '17'
AND fa.SpeciesActivityKey = '6'
