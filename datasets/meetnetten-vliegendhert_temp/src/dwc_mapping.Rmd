...
```{r}
input_data <- read_delim(here::here("datasets","meetnetten-vliegendhert_temp","data","interim","transectobservationsGBIF_refined.csv"), ",")
...
```{r}
occurrence %<>% mutate(dwc_rightsHolder = rights_holder)
```
...
```{r}
occurrence %<>% mutate(dwc_recordedBy = hash(visit_user_name))
```
...
```{r}
occurrence %<>% mutate(dwc_identifiedBy = visit_user_name)
```
...
```{r}
write_csv(occurrence, here::here("datasets","meetnetten-vliegendhert_temp","data","processed","occurrence.csv"), na = "")
```
